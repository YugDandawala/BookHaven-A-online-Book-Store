function displaySciFiSellers() {
    const container = document.querySelector('#scifiseller .row');
    if (!container) return;

    container.innerHTML = ""; // Clear any old content

    const scifiBooks = allBooks.filter(b => b.id.startsWith("S")).slice(0, 8);
...
document.addEventListener("DOMContentLoaded", displaySciFiSellers);
